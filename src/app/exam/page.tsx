...
const MOCK = [["001","Mid-Term Test","2026-A","2026-09-05","Active"],["002","Unit Test 1","2026-B","2026-08-25","Active"],["003","Final Exam","2026-A","2026-12-10","Inactive"]];
  ...
  return (
    <div className={s.page}>
      <div className={s.pageHeader}>
        <div>
          <h1 className={s.pageTitle}>Examinations</h1>
          <p className={s.pageDesc}>Schedule and manage examinations for all batches.</p>
        </div>
      ...
      <div className={s.card}>
        ...
        <div className={s.tableWrap}>
          <table className={s.table}>
            <thead><tr><th>#</th><th>Exam Name</th><th>Class</th><th>Date</th><th>Status</th><th></th></tr></thead>
            <tbody>
              {filtered.map((row, i) => (
                <tr key={i}>
      ...
      <Modal isOpen={open} onClose={() => setOpen(false)} title="Add New Record">
        <form onSubmit={handleAdd} className={s.form}>
          <Input label="Exam Name" value={newVal} onChange={e => setNewVal(e.target.value)} required />
